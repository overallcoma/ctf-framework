...
COPY CTF-MultiFactorNumberCruncher-02-v2-Launcher.py /app
COPY CTF-MultiFactorNumberCruncher-02-v2-WebSetup.py /app
COPY CTF-MultiFactorNumberCruncher-02-v2-PythonLoop.py /app
COPY flag.txt /app
RUN apk update
...
RUN touch httpd.pid
EXPOSE 80
CMD ["python3", "/app/CTF-MultiFactorNumberCruncher-02-v2-Launcher.py"]
"""
file = open('dockerfile', 'w+')
file.write(dockerfile)
file.close()
call(["docker", "build", "-t", "ctrl/ctf-mfanumbercruncher-02-v2:latest", "."])
containername = "CTF-MFANumberCruncher-02-v2-" + str(secretkey)
call(["docker", "run", "-d", "--name", containername, "ctrl/ctf-mfanumbercruncher-02-v2"])
remove('seed.txt')
remove('dockerfile')
remove('flag.txt')
