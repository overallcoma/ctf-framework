...
COPY CTF-BasicWeb-03-Launcher.py /app
...
RUN touch httpd.pid
EXPOSE 80
CMD ["python3", "/app/CTF-BasicWeb-03-Launcher.py"]
"""

file = open('dockerfile', 'w+')
file.write(dockerfile)
file.close()
call(["docker", "build", "-t", "ctrl/ctf-basicweb-03:latest", "."])
containername = "CTF-BasicWeb-03-" + str(password)
flag_env_variable = "FLAG=" + flag
password_env_variable = "PASSWORD=" + password
call(["docker", "run", "-d", "--name", containername, "-e", flag_env_variable, "-e", password_env_variable, "ctrl/ctf-basicweb-03"])
remove('dockerfile')
